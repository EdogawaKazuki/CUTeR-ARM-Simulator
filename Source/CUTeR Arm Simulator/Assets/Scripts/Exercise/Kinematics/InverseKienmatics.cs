...
public class InverseKienmatics : MonoBehaviour
{
    ...
    public void UpdateTable()
    {
        ...
        matrix1Text[0].text = (angle1 + 0.00001f).ToString("F1") + "бу";
        matrix1Text[3].text = (angle1 + 0.00001f).ToString("F1") + "бу";
        if (angle1 > RobotController.Sliders[0].maxValue || angle1 < RobotController.Sliders[0].minValue)
        {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            {  //if it's a real solution
                ...
                if (n == 0)
                {
                    matrix1Text[2].text = angle3_deg[0].ToString("F1") + "бу";

                    if (angle3_deg[0] > RobotController.Sliders[2].maxValue || angle3_deg[0] < RobotController.Sliders[2].minValue)
                    {
                ...
                {
                    matrix1Text[5].text = angle3_deg[1].ToString("F1") + "бу";

                    if (angle3_deg[1] > RobotController.Sliders[2].maxValue || angle3_deg[1] < RobotController.Sliders[2].minValue)
                    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (i == 0)
            {
                matrix1Text[1].text = angle2_deg[0].ToString("F1") + "бу";

                if (angle2_deg[0] > RobotController.Sliders[1].maxValue || angle2_deg[0] < RobotController.Sliders[1].minValue)
                {
            ...
            {
                matrix1Text[4].text = angle2_deg[1].ToString("F1") + "бу";

                if (angle2_deg[1] > RobotController.Sliders[1].maxValue || angle2_deg[1] < RobotController.Sliders[1].minValue)
                {
    ...
    public void Select1()
    {
        RobotController.JointAngle[0] = angle1;
        RobotController.JointAngle[1] = angle2_deg[0];
        RobotController.JointAngle[2] = angle3_deg[0];

    }
    public void Select2()
    {
        RobotController.JointAngle[0] = angle1;
        RobotController.JointAngle[1] = angle2_deg[1];
        RobotController.JointAngle[2] = angle3_deg[1];

...
#region property
    ...
    /// 坐标轴缩放的速度
    /// </summary>
    private Vector2 m_ZoomSpeed = new Vector2(1, 5000);

    /// <summary>
    /// 坐标轴移动的速度
    /// </summary>
    private Vector2 m_MoveSpeed = new Vector2(0, 1);

    /// <summary>
    /// 坐标轴最大可伸缩范围，以坐标轴为单位
    /// Y轴的通过长宽比例计算获得
    /// </summary>
    private float m_CoordinateAxisMaxWidth = 50000;
    private float m_CoordinateAxisMinWidth = 100;
    ...
    private float m_PixelPerMark {
        get { return INCH_PER_CENTIMETER * m_DataDiagram.m_CentimeterPerMark * ScreenDpi; }
    }
    private int ScreenDpi = 243;
    /// <summary>
    /// point表示坐标轴刻度值的零点位置
    ...
    private Rect m_CoordinateAxisRange {
        get {
            try {
                ...
                return new Rect(0, 0,
                    sizePixel.x / (m_DataDiagram.m_CentimeterPerCoordUnitX * INCH_PER_CENTIMETER * ScreenDpi),
                    sizePixel.y / (m_DataDiagram.m_CentimeterPerCoordUnitY * INCH_PER_CENTIMETER * ScreenDpi));
...
#region delegate
    ...
    protected override void Awake() {
        ...
        m_CoordinateAxisViewRange.position = new Vector2(0,0);
        m_CoordinateAxisViewRange.size = new Vector2(2, 20000);
    ...
    {
        m_CoordinateAxisViewRange.position = position;
    }
    public void SetViewRange(Vector2 size)
    {
        m_CoordinateAxisViewRange.size = size;
    }
...
#region draw rect coordinateAxis
    ...
    public void InputPoint(GameObject line, Vector2 point) {
        ...
        line.GetComponent<DD_Lines>().AddPoint(point, CoordinateToPixel(point));
    }
    ...
    public int GetLinePos(int index)
    {
        return m_LineList[index].GetComponent<DD_Lines>().GetPointByIndex(0);
    }

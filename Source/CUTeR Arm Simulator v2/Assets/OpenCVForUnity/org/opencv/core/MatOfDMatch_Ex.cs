...
namespace OpenCVForUnity.CoreModule
{
    public partial class MatOfDMatch : Mat
    {
        ...
        public MatOfDMatch(ReadOnlySpan<Vec4f> a)
            : base()
        {
            fromVec4fSpan(a);
        }
        ...
        public MatOfDMatch(params (float queryIdx, float trainIdx, float imgIdx, float distance)[] a)
            : base()
        {
        ...
        public void fromValueTupleArray(params (float queryIdx, float trainIdx, float imgIdx, float distance)[] a)
        {
            ...
            if (this.isContinuous())
            {
                ...
                for (int i = 0; i < span.Length; i++)
                {
                    ...
                    vec_span.Item1 = tuple_a.queryIdx;
                    vec_span.Item2 = tuple_a.trainIdx;
                    vec_span.Item3 = tuple_a.imgIdx;
                    vec_span.Item4 = tuple_a.distance;
                }
            ...
            if (this.isContinuous())
            {
                unsafe
                {
                    ...
                    {
                        ref readonly var tuple_a = ref a[i];
                        ptr[i * 4] = tuple_a.queryIdx;
                        ptr[i * 4 + 1] = tuple_a.trainIdx;
                        ptr[i * 4 + 2] = tuple_a.imgIdx;
                        ptr[i * 4 + 3] = tuple_a.distance;
                    }
                }
            ...
            {
                ref readonly var tuple_a = ref a[i];
                buff[i * 4] = tuple_a.queryIdx;
                buff[i * 4 + 1] = tuple_a.trainIdx;
                buff[i * 4 + 2] = tuple_a.imgIdx;
                buff[i * 4 + 3] = tuple_a.distance;
            }
        ...
        public (float queryIdx, float trainIdx, float imgIdx, float distance)[] toValueTupleArrayAsFloat()
        {
            ...
            if (this.isContinuous())
            {
                ...
                for (int i = 0; i < span.Length; i++)
                {
                    ...
                    tuple_ap.queryIdx = vec_span.Item1;
                    tuple_ap.trainIdx = vec_span.Item2;
                    tuple_ap.imgIdx = vec_span.Item3;
                    tuple_ap.distance = vec_span.Item4;
                }
            ...
            if (this.isContinuous())
            {
                unsafe
                {
                    ...
                    {
                        ref var tuple_ap = ref ap[i];
                        tuple_ap.queryIdx = ptr[i * 4];
                        tuple_ap.trainIdx = ptr[i * 4 + 1];
                        tuple_ap.imgIdx = ptr[i * 4 + 2];
                        tuple_ap.distance = ptr[i * 4 + 3];
                    }
                }
            ...
            {
                ref var tuple_ap = ref ap[i];
                tuple_ap.queryIdx = buff[i * 4];
                tuple_ap.trainIdx = buff[i * 4 + 1];
                tuple_ap.imgIdx = buff[i * 4 + 2];
                tuple_ap.distance = buff[i * 4 + 3];
            }
#endif

    ...
    public readonly struct PoseEstimationCOCO17Data
    {
        ...
        public PoseEstimationCOCO17Data(float x1, float y1, float x2, float y2, float confidence, int classId, Vec3f[] landmarks)
        {
            ...
            for (int i = 0; i < landmarks.Length; i++)
            {
                ...
                _rawLandmarks[offset + 0] = landmark.Item1;
                _rawLandmarks[offset + 1] = landmark.Item2;
                _rawLandmarks[offset + 2] = landmark.Item3;
            }
        }
        ...
        public readonly Vec3f[] GetLandmarksArray()
        {
            Vec3f[] landmarks = new Vec3f[LANDMARK_VEC3F_COUNT];
            for (int i = 0; i < landmarks.Length; i++)
            {
                int offset = i * 3;
                landmarks[i] = new Vec3f(_rawLandmarks[offset + 0],
                                             _rawLandmarks[offset + 1],
                                             _rawLandmarks[offset + 2]);
            }
            return landmarks;
        }
...
#else
            Vec3f[] landmarks = GetLandmarksArray();
#endif
            sb.Append("Landmarks:");
            for (int i = 0; i < landmarks.Length; i++)
            {
